  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto py-20 px-4 md:px-8 lg:px-10">
        <h2 className="text-4xl md:text-5xl font-black uppercase tracking-tight text-foreground mb-4 max-w-4xl">
          Everything you need, <span className="text-blue-600">Simplified.</span>
        </h2>
        <p className="text-muted-foreground text-sm md:text-base max-w-sm font-medium">
          HomeCrew is designed to be effortless. Here is exactly how we get your projects done from start to finish.
        </p>
      </div>
        ...
        {data.map((item, index) => (
          ...
          >
            <div className="sticky flex flex-col md:flex-row z-40 items-center top-40 self-start max-w-xs lg:max-w-sm md:w-full">
              ...
              <h3 className="hidden md:block text-xl md:pl-20 md:text-5xl font-black uppercase text-muted-foreground/30 ">
                {item.title}
              </h3>
            </div>

            <div className="relative pl-20 pr-4 md:pl-4 w-full">
              <h3 className="md:hidden block text-2xl mb-4 text-left font-black uppercase text-muted-foreground/30">
                {item.title}
              </h3>
              {item.content}{" "}
            </div>
